...
export default function App() {
  ...
  const fetchVideoInfo = async (url) => {
    ...
    try {
      const { data } = await api.post('/api/youtube/info', { url })
      if (data) {
        setVideoInfo(data)
        setVideoError('')
      }
      ...
      const errorMessage = err?.response?.data?.message || err?.message || 'Could not fetch video information'
      setVideoError(errorMessage)
      // Even if there's an error, try to show at least the thumbnail if we have a video ID
  ...
  // Debounce YouTube link fetching
  useEffect(() => {
    if (!form.youtubeLink) {
      setVideoInfo(null)
      setVideoError('')
    ...
    const timeoutId = setTimeout(() => {
      fetchVideoInfo(form.youtubeLink)
  ...
  const calculateCustomPricing = async (price) => {
    ...
    setPricingData(null)
    try {
      console.log('Calculating pricing for:', price)
      const { data } = await api.post('/api/pricing/calculate', { price })
  ...
  // Calculate pricing when custom package is selected
  useEffect(() => {
    if (form.selectedPackage === 'custom') {
      // Calculate immediately when custom is first selected
      calculateCustomPricing(customPrice)
    } else {
      setPricingData(null)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // Debounce pricing calculation when price changes (only if custom is selected)
  useEffect(() => {
    if (form.selectedPackage === 'custom') {
      const timeoutId = setTimeout(() => {
        calculateCustomPricing(customPrice)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.selectedPackage === 'custom') {
        if (!pricingData) {
          setError('Please wait for pricing calculation to complete')
          setLoading(false)
          return
        }
        plan = {
          name: 'Custom Package',
          type: 'views',
          quantity: pricingData.totalViews.exact,
          price: customPrice,
          currency: 'INR',
        }
      } else {
          ...
          <div className="border rounded p-4 bg-blue-50">
               ...
               <div className="mt-3 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                 <div className="flex items-start space-x-4">
                   ...
                   <div className="flex-1">
                     ...
                     {videoInfo.author && (
                       <p className="text-sm text-gray-600 mb-2">by {videoInfo.author}</p>
                     )}
                     <a
           ...
           <div className="space-y-4">
             ...
             <div className="grid md:grid-cols-4 gap-4">
               ...
               >
                 <div className="text-center">
                   <h4 className="text-lg font-bold text-gray-900 mb-2">Custom Price</h4>
                   <p className="text-sm text-gray-600 mb-3">Set your own budget</p>
                   <div className="text-2xl font-bold text-purple-600 mb-3">₹10 - ₹10K</div>
                   <div className="text-sm text-gray-500 mb-4">Flexible Pricing</div>
                   <ul className="text-xs text-gray-600 space-y-1">
           ...
           {form.selectedPackage === 'custom' && (
             <div className="mt-6 p-6 bg-gradient-to-br from-purple-50 to-blue-50 border-2 border-purple-200 rounded-lg">
               <h3 className="text-xl font-bold text-gray-900 mb-6">💰 Budget & Targeting</h3>
               
               {/* Price Slider */}
               <div className="mb-6">
                 <div className="flex items-center justify-between mb-4">
                   <label className="text-sm font-semibold text-gray-700">Enter Budget</label>
                   <div className="px-4 py-2 bg-purple-600 text-white rounded-full font-bold text-lg">
                 ...
                 <input
                   ...
                   style={{
                     background: `linear-gradient(to right, #9333ea 0%, #9333ea ${((customPrice - 10) / (10000 - 10)) * 100}%, #e5e7eb ${((customPrice - 10) / (10000 - 10)) * 100}%, #e5e7eb 100%)`
                   }}
                 />
               ...
               {loadingPricing ? (
                 ...
                 <div className="bg-white rounded-lg p-6 shadow-md">
                   <h4 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                     Estimated Views
                     <span className="ml-2 text-gray-400 cursor-help" title="Base views you'll receive">ℹ️</span>
                   </h4>
                   ...
                   <div className="mb-4">
                     <div className="flex h-8 rounded-lg overflow-hidden mb-2">
                       <div 
                         ...
                         style={{ width: `${(pricingData.baseViews.exact / pricingData.totalViews.exact) * 100}%` }}
                       >
                         {pricingData.baseViews.min.toLocaleString()} - {pricingData.baseViews.max.toLocaleString()}
                       </div>
                       <div 
                         className="bg-green-500 flex items-center justify-center text-white text-xs font-semibold"
                         style={{ width: `${(pricingData.bonusViews.exact / pricingData.totalViews.exact) * 100}%` }}
                       >
                         {pricingData.bonusViews.min.toLocaleString()} - {pricingData.bonusViews.max.toLocaleString()}
                       </div>
                     </div>
                     <div className="flex justify-between text-xs text-gray-600">
                       <span>Base Views</span>
                       <span>Bonus ({pricingData.bonusViews.percentage}%)</span>
                     </div>
                   </div>
                   ...
                   <div className="border-t pt-4">
                     <div className="flex justify-between items-center mb-2">
                       ...
                       <span className="text-2xl font-bold text-purple-600">
                         {pricingData.totalViews.min.toLocaleString()} - {pricingData.totalViews.max.toLocaleString()}
                       </span>
                     </div>
                   </div>
                 </div>
                 ...
                 <div className="text-center py-8 text-gray-500">
                   Adjust the slider to see estimated views
                 </div>
               )}
             </div>
           )}
