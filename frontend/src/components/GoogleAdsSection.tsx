...
const GoogleAdsSection = ({ showBadge = true, bgColor = "#ffffff" }: GoogleAdsSectionProps) => {
  const stats = [
    {
      icon: (
  ...
  return (
    <section
      className="font-founders py-14 lg:py-20"
      style={{ backgroundColor: bgColor }}
    >
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        {/* Stats Row */}
        <div className={`flex flex-col sm:flex-row justify-between gap-10 ${showBadge ? 'mb-16 lg:mb-20' : ''}`}>
          {stats.map((stat, index) => (
            <div key={index} className="flex items-center gap-4">
              <div className="flex-shrink-0 w-[64px] h-[64px] rounded-[14px] border border-gray-200 bg-white flex items-center justify-center shadow-sm">
                {stat.icon}
              </div>
              <div>
                ...
                <div className="text-[12px] font-bold text-[#475569] tracking-wider mt-1 uppercase">
                  {stat.label}
                </div>
              </div>
            </div>
